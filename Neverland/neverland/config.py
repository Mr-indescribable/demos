...
from neverland.utils import VerifiTools
...
class ConfigLoader():
    ...
    def validate_config(cls, config):
        basic = config.basic
        if not VerifiTools.type_matched(basic, OD):
        ...
        crypto = net.crypto
        if not VerifiTools.type_matched(crypto, OD):
        ...
        shm = config.shm
        if not VerifiTools.type_matched(shm, OD):
        ...
        if basic.role != Roles.CONTROLLER:
            ...
            if not VerifiTools.type_matched(cluster_entrance, OD):
            ...
            if not VerifiTools.type_matched(cluster_nodes, OD):
